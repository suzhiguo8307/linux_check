...
echo " "
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>身份鉴别安全<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
grep -i "^password.*requisite.*pam_cracklib.so" /etc/pam.d/system-auth 
if [ $? == 0 ];then
    echo ">>>密码复杂度:已设置"
else
...
more /etc/login.defs | grep  "^PASS_MIN_DAYS" 
more /etc/login.defs | grep  "^PASS_MIN_LEN"
more /etc/login.defs | grep  "^PASS_WARN_AGE"
echo "=============================dividing line================================"
grep -i "^auth.*required.*pam_tally2.so.*$" /etc/pam.d/sshd 
if [ $? == 0 ];then
  echo ">>>登入失败处理:已开启"
else
...
echo "=============================dividing line================================"
echo "查看audit审计服务是否开启"
if service auditd status | egrep " active \(running";then
  echo ">>>经分析,audit服务已开启"
else
  echo ">>>经分析audit服务未开启，建议通过service audit start开启日志审计功能---------[需调整]"
fi
echo "=============================dividing line================================"
